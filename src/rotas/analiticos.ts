...
rota.get("/pedidos-por-dia", async (_req, res) => {
  const now = new Date();
  const inicio = new Date(now); inicio.setDate(now.getDate()-13);
  const data = await prisma.pedido.findMany({ where: { criado_em: { gte: inicio } }, select: { criado_em: true } });
  const mapa: Record<string, number> = {};
  for(let i=0;i<14;i++){ const d=new Date(inicio); d.setDate(inicio.getDate()+i); mapa[d.toISOString().slice(0,10)]=0; }
  for(const o of data){ const key=o.criado_em.toISOString().slice(0,10); if(mapa[key]!=null) mapa[key]++; }

...
rota.post("/pedido/:pedidoId", verificarToken, async (req, res) => {
    ...
    if (pedido.status !== StatusPedido.ENTREGUE)
        return res.status(400).json({ erro: "Só é possível avaliar pedidos entregues" });
    const ja = await prisma.avaliacao.findUnique({ where: { pedido_id: pedidoId } });
    if (ja)
        return res.status(400).json({ erro: "Pedido já avaliado" });
    const rv = await prisma.avaliacao.create({ data: { pedido_id: pedidoId, usuario_id: req.usuario.id, restaurante_id: pedido.restaurante_id, nota: parsed.data.nota, comentario: parsed.data.comentario } });
    res.json(rv);
});
export default rota;

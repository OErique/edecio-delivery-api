...
rota.get("/", async (_req, res) => {
    ...
    const comMedia = restaurantes.map((r) => ({
        ...r,
        media: r.avaliacoes.length ? (r.avaliacoes.reduce((s, x) => s + x.nota, 0) / r.avaliacoes.length) : null
    }));
    res.json(comMedia);
});
...
rota.post("/", verificarToken, exigirPapel("ADMIN"), async (req, res) => {
    ...
    res.json(await prisma.restaurante.create({ data: parsed.data }));
});
